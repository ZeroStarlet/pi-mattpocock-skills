...
Each harness excludes a user-invoked skill from the model's reach in its own way, so nothing but the human can fire it: no other skill can. PI uses the same `disable-model-invocation` frontmatter as Claude Code, and the native PI package's `skill` tool enforces it when composing skills. A user-invoked skill may invoke model-invoked skills, but it can never reach another user-invoked skill.
